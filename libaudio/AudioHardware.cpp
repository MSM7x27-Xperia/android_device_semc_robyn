...
//#define LOG_NDEBUG 0
#define LOG_TAG "AudioHardwareSEMC"
#include <utils/Log.h>
...
AudioHardware::AudioHardware() :
    ...
    mOutput(0), mSndEndpoints(NULL), mCurSndDevice(-1), mDualMicEnabled(false), mBuiltinMicSelected(true)
{
...
status_t AudioHardware::doRouting(AudioStreamInMSM72xx *input)
{
    ...
    if (input != NULL) {
        ...
        if (inputDevice != 0) {
            if (inputDevice & AudioSystem::DEVICE_IN_BLUETOOTH_SCO_HEADSET) {
                LOGI("Paul lib routing audio to Bluetooth PCM\n");
                new_snd_device = SND_DEVICE_BT;
            } else if (inputDevice & AudioSystem::DEVICE_IN_WIRED_HEADSET) {
                    LOGI("Paul lib routing audio to Wired Headset\n");
                    new_snd_device = SND_DEVICE_HEADSET;
            } else {
                if (outputDevices & AudioSystem::DEVICE_OUT_SPEAKER) {
                    LOGI("Paul lib routing audio to Speakerphone\n");
                    new_snd_device = SND_DEVICE_FARFIELD_CL;
                    new_post_proc_feature_mask = (ADRC_ENABLE | EQ_ENABLE | RX_IIR_ENABLE | MBADRC_ENABLE);
                } else {
                    LOGI("Paul lib routing audio to Handset\n");
    ...
    if (new_snd_device == -1) {
        ...
        if ((mTtyMode != TTY_OFF) && (mMode == AudioSystem::MODE_IN_CALL) &&
            ...
            if (mTtyMode == TTY_FULL) {
                LOGI("Paul lib routing audio to TTY FULL Mode\n");
                new_snd_device = SND_DEVICE_TTY;
            } else if (mTtyMode == TTY_VCO) {
                LOGI("Paul lib routing audio to TTY VCO Mode\n");
                new_snd_device = SND_DEVICE_VCO;
            } else if (mTtyMode == TTY_HCO) {
                LOGI("Paul lib routing audio to TTY HCO Mode\n");
            ...
            LOGI("Paul lib routing audio to Wired Headset and Speaker\n");
            new_snd_device = SND_DEVICE_FARFIELD_HEADSET;
            new_post_proc_feature_mask = (ADRC_ENABLE | EQ_ENABLE | RX_IIR_ENABLE | MBADRC_ENABLE);
        } else if (outputDevices & AudioSystem::DEVICE_OUT_WIRED_HEADPHONE) {
            if (outputDevices & AudioSystem::DEVICE_OUT_SPEAKER) {
                LOGI("Paul lib routing audio to No microphone Wired Headset and Speaker (%d,%x)\n", mMode, outputDevices);
                new_snd_device = SND_DEVICE_FARFIELD_HEADSET;
                new_post_proc_feature_mask = (ADRC_ENABLE | EQ_ENABLE | RX_IIR_ENABLE | MBADRC_ENABLE);
            } else {
                LOGI("Paul lib routing audio to No microphone Wired Headset (%d,%x)\n", mMode, outputDevices);
                new_snd_device = SND_DEVICE_HEADSET;
                new_post_proc_feature_mask = (ADRC_ENABLE | EQ_ENABLE | RX_IIR_ENABLE | MBADRC_ENABLE);
            }
#endif
            ...
            LOGI("Paul lib re-routing audio to Speakerphone\n");
            new_snd_device = SND_DEVICE_FARFIELD_CL;
            new_post_proc_feature_mask = (ADRC_ENABLE | EQ_ENABLE | RX_IIR_ENABLE | MBADRC_ENABLE);
        } else {
            LOGI("Paul lib routing audio to Handset\n");
    ...
    if (mDualMicEnabled && mMode == AudioSystem::MODE_IN_CALL) {
        if (new_snd_device == SND_DEVICE_HANDSET_CL) {
            LOGI("Paul lib routing audio to handset with DualMike enabled\n");
            new_snd_device = SND_DEVICE_IN_S_SADC_OUT_HANDSET;
        } else if (new_snd_device == SND_DEVICE_FARFIELD_CL) {
            LOGI("Paul lib routing audio to speakerphone with DualMike enabled\n");
